...
printf("value of ptr == %p\n", ptr);    //포인터 변수ptr이 변수i의 메모리 주소를 가리키고 있으므로 변				     수 i의 주소를 출력
printf("address of ptr == %p\n", &ptr); //포인터 변수ptr의 주소는 그대로 이므로 포인터 변수ptr의 주소				     출력
printf("value of *ptr == %d\n", *ptr);  //포인터 변수ptr이 가리키는 메모리 공간에 있는 값 출력 (변수i				     의 값)
...
printf("value of *ptr == %d\n", *ptr);  //포인터 변수 ptr이 가리키는 공간이 달라지지 않았으므로 변수i				     의 값인 1234 출력
printf("value of dptr == %p\n", dptr);  //이중포인터 변수 dptr이 포인터변수 ptr의 메모리공간을 가리키				     고 있으므로 포인터변수ptr의 메모리 주소값 출력
...
printf("value of **dptr == %d\n", **dptr);  //이중포인터dptr이 가리키는 변수인 포인터변수ptr이 가리키					는 공간의 값인 7777출력
**dptr = 8888; /* changing the value of **dptr */
printf("\n[after **dptr = 8888] \n");//이중포인터 변수dptr의 역참조인 포인터변수ptr의 역참조 값이 				     8888로 바뀜
printf("value of i == %d\n", i);//변수 i는 이중포인터 변수dptr의 역참조의역참조 이므로 값이 8888로 바			     뀌었으므로 8888출력
printf("value of *ptr == %d\n", *ptr);//포인터 변수ptr은 이중포인터 변수dptr의 역참조이므로 8888출력
printf("value of **dptr == %d\n", **dptr);//이중포인터 변수dptr의 역참조인 포인터변수ptr의 역참조 값					인 8888출력
return 0;
}
